			...
			return jq.each(function(){
				...
				var droppableOptions = {
					...
					onDragLeave: function(e, source) {
						...
						tr.children('td').css('border-width','0 1px 1px 0');
						tr.children('td').css('border-style','dotted');
						tr.children('td').css('border-color','#D3D3D3'); 
				...
				function getDraggingRow(tr){
					...
					var target = $(tr).closest('div.datagrid-view').children('table')[0];
					var opts = $(target).datagrid('options');
					var tmp = $(target).datagrid('getSelections');
					var isRows = (tmp && tmp.length >1);
					if (isRows || opts.dragSelection){
